...
public class ScionHeaderParser {
  ...
  private static int extractDstPort(
      ByteBuffer data, int nextHdrOffset, InternalConstants.HdrTypes hdrType) {
    int dstPort;
    if (hdrType == InternalConstants.HdrTypes.UDP) {
      // get remote port from UDP overlay
      data.position(nextHdrOffset + 2);
      dstPort = Short.toUnsignedInt(data.getShort());
    } else if (hdrType == InternalConstants.HdrTypes.SCMP) {
      // get remote port from SCMP header
      data.position(nextHdrOffset);
  ...
  public static String validate(ByteBuffer data) {
    ...
    int hdrLen = ByteUtil.readInt(i1, 8, 8);
    int hdrLenBytes = hdrLen * 4;
    int payLoadLen = ByteUtil.readInt(i1, 16, 16);
  ...
  public static void reversePathInPlace(ByteBuffer data) {
    ...
    int i0R = 0;
    i0R = ByteUtil.writeInt(i0R, 0, 2, 0); // CurrINF = 0
    i0R = ByteUtil.writeInt(i0R, 2, 6, 0); // CurrHF = 0
    i0R = ByteUtil.writeInt(i0R, 8, 6, 0); // RSV = 0
    // reverse segLen
