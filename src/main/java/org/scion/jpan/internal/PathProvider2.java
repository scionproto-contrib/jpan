...
public interface PathProvider2<K> {
  ...
  /**
   * Register a callback that is invoked when paths are updated. A typical single-path
   * DatagramChannel will require only one such callback. If the PAthProvider is connected, see
   * {@link #connect(Path)}, it will immediately invoke the callback with a path.
   *
