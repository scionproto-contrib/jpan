...
public class SegmentsNew {
  private static final Logger LOG = LoggerFactory.getLogger(SegmentsNew.class.getName());
    ...
    if (!localAS.isCoreAs()) {
      ...
      if (minimizeLookups) {
        ...
        if (!directUp.isEmpty()) {
          ...
          PathDuplicationFilterNew paths = new PathDuplicationFilterNew();
  ...
  private static void combineSegment(
      PathDuplicationFilterNew paths,
      List<PathSegment> segments,
      LocalAS localAS,
      long srcIsdAs,
      long dstIsdAs) {
  ...
  private static void buildPath(
      PathDuplicationFilterNew paths, LocalAS localAS, long dstIsdAs, PathSegment... segments) {
    PathMetadata.Builder path = PathMetadata.newBuilder();
    ...
    // Metadata
    SegmentMetadataAccumulatorNew.writeStaticInfoMetadata(path, segments, ranges);
